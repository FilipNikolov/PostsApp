...
import {Post, User, SingleComment, FullPost} from "./types";
...
const Posts = () => {
 ...
 const fetchData = async () => {
  ...
  function getMergedData(posts:Post[], users:User[], comments:SingleComment[]):FullPost[] {                          
    let all:FullPost[] = posts.map((post) => {
  ...
  const handleChange = (e:any) => {
    ...
    let filterData = data.filter((item) => item.user?.name.toLowerCase().includes(e.target.value))
              ...
              <div className="card-body post-card ">
                <Link to={`/post/${item.post.id}`}>
                  <section key={item.post.id}>
                    <h3>{item.post.title} {item.post.id}</h3><hr/>
                    <span><strong>User: </strong>{item.user?.name}</span><br/>
                    <span><strong>Body: </strong>{item.post.body}</span>
                  </section>
                  </Link>  
                  <div id="comments">
                    <main>
